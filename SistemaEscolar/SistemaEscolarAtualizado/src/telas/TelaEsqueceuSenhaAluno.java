...
public class TelaEsqueceuSenhaAluno {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					TelaEsqueceuSenhaAluno window = new TelaEsqueceuSenhaAluno();
	...
	private void initialize() {
		...
		lblMatricula.setBounds(31, 142, 102, 14);
		...
		lblNovaSenha.setBounds(31, 204, 102, 14);
				...
				if(cpf.getText().isEmpty()|| String.valueOf(senha.getPassword()).isEmpty()|| String.valueOf(senhaRpt.getPassword()).isEmpty() || matricula.getText().isEmpty()) {
					...
					if(redefinirEscolha == JOptionPane.YES_OPTION) {
						...
						if(!Aluno.verificarCpfExiste(cpfLido)) {
							...
							Aluno aluno = Aluno.getAlunoObjeto(matriculaLida);
							if(aluno.getCpf().equals(cpfLido)) {
